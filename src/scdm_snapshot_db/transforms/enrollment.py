...
def filter_medical_and_drug(enrollment_df: DataFrame) -> DataFrame:
    ...
    return enrollment_df.filter(
        (F.lower(F.col("drugcov")) == "y") & (F.lower(F.col("medcov")) == "y")
    )
...
def bridge_intervals(df: DataFrame) -> DataFrame:
    ...
    prior_max_end = F.max("enr_end").over(w.rowsBetween(-9223372036854775808, -1))
    ...
    span_id = F.sum(new_span).over(w.rowsBetween(-9223372036854775808, 0))

    with_spans = cleaned.withColumn("_span_id", span_id)

    # Aggregate each patient/span to min start / max end
    return (
        with_spans.groupBy("patid", "_span_id")
        .agg(
...
def enr_pat_covlength_md(bridged_md: DataFrame, dp: str) -> DataFrame:
    ...
    """
    total_length = (
        bridged_md.withColumn("span_days", F.datediff(F.col("_enr_end"), F.col("_enr_start")) + 1)
        .groupBy("patid")
        .agg(F.sum("span_days").alias("total_length"))
    )
    return (
        total_length.groupBy(
            F.lit(dp).alias("dp"),
...
def enr_patid_ct_md(bridged_md: DataFrame, dp: str) -> DataFrame:
    ...
    return (
        distinct_patids.groupBy(F.lit(dp).alias("dp"))
        .agg(F.count(F.lit(1)).cast("long").alias("count"))
        .select("dp", "count")
    )
...
def enr_pat_covyears_md(bridged_md: DataFrame, dp: str) -> DataFrame:
    ...
    """
    with_years = (
        bridged_md.withColumn("start_year", F.year("_enr_start"))
    ...
    return (
        with_years.groupBy(F.lit(dp).alias("dp"), F.col("year").cast("int").alias("year"))
        .agg(F.count(F.lit(1)).cast("long").alias("count"))
...
def enr_pat_enrcount_md(bridged_md: DataFrame, dp: str) -> DataFrame:
    ...
    """
    span_counts = bridged_md.groupBy("patid").agg(
        F.countDistinct("_span_id").cast("long").alias("enr_count")
    )
    return (
        span_counts.groupBy(
            F.lit(dp).alias("dp"),
...
def enr_active_patid_ct_md(
    ...
    """
    active = bridged_md.filter(F.col("_enr_end") >= F.lit(dp_max_date)).select("patid").distinct()
    return (
        active.groupBy(F.lit(dp).alias("dp"))
        .agg(F.count(F.lit(1)).cast("long").alias("count"))
        .select("dp", "count")
    )
